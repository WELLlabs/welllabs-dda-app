...
describe('accounts api', () => {
	...
	it('register posts credentials to /auth/register', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/auth/register', {
			credentials: 'include',
			method: 'POST',
	...
	it('login posts form-urlencoded credentials then loads /me', async () => {
		...
		expect(fetch).toHaveBeenNthCalledWith(1, '/wst/backend/accounts/auth/login', {
			credentials: 'include',
			method: 'POST',
		...
		expect(fetch).toHaveBeenNthCalledWith(2, '/wst/backend/accounts/auth/me', { credentials: 'include' });
	});

	it('logout posts to /auth/logout', async () => {
		fetch.mockResolvedValue({ ok: true, status: 204 });
		await logout();
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/auth/logout', {
			credentials: 'include',
			method: 'POST'
		});
	});
	...
	it('me fetches the current user', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/auth/me', { credentials: 'include' });
	});
	...
	it('updateMe patches /auth/users/me', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/auth/users/me', {
			credentials: 'include',
			method: 'PATCH',
			headers: { 'Content-Type': 'application/json' },
	...
	it('verify / request-verify / forgot / reset hit auth endpoints', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/auth/verify', {
			credentials: 'include',
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/auth/request-verify-token', {
			credentials: 'include',
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/auth/forgot-password', {
			credentials: 'include',
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ email: 'a@b.com' })
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/auth/reset-password', {
			credentials: 'include',
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
	...
	it('lookupUserByEmail encodes the email query', async () => {
		...
		expect(fetch).toHaveBeenCalledWith(
			'/wst/backend/accounts/users/lookup?email=ada%20lovelace%40example.com',
			{ credentials: 'include' }
		);
	});
	...
	it('createOrg posts the org name', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/orgs', {
			credentials: 'include',
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
	...
	it('fetchOrgMembers returns members array', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/orgs/o1/members', {
			credentials: 'include'
		});
	});
	...
	it('removeOrgMember and deleteOrg issue DELETE requests', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/orgs/o1/members/u1', {
			credentials: 'include',
			method: 'DELETE'
		});

		await deleteOrg('o1');
		expect(fetch).toHaveBeenCalledWith('/wst/backend/accounts/orgs/o1', {
			credentials: 'include',
			method: 'DELETE'
		});
	});
