...
describe('diagnose api', () => {
	...
	it('fetchProjects and fetchProject hit project endpoints', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/diagnose/projects/p1', { credentials: 'include' });
	});
	...
	it('deleteProject issues DELETE', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/diagnose/projects/p1', { credentials: 'include', method: 'DELETE' });
	});
	...
	it('user access helpers read and mutate members', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/diagnose/projects/p1/access/users', {
			credentials: 'include',
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/diagnose/projects/p1/access/users/u1', {
			credentials: 'include',
			method: 'DELETE'
		});
	});
	...
	it('org access helpers read and mutate org grants', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/diagnose/projects/p1/access/orgs', {
			credentials: 'include',
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/diagnose/projects/p1/access/orgs/o1', {
			credentials: 'include',
			method: 'DELETE'
		});
	});
	...
	it('lookupWatershed posts coordinates', async () => {
		...
		expect(fetch).toHaveBeenCalledWith('/wst/backend/diagnose/watersheds/lookup', {
			credentials: 'include',
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
