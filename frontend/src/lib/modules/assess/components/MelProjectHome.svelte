...
<div class="relative min-h-screen bg-transparent font-body">
	...
	<main class="relative z-10 flex-1 overflow-auto p-6">
		...
		{:else if plans.length === 0}
			...
			>
				No MEL plans yet.
				<button type="button" class="text-[#16a34a] underline" onclick={openCreate}>Add a plan</button>
				to get started.
			</div>
		{:else}
			<div
			...
			>
				{#each plans as plan, i (plan.id)}
					<div
						class="card group"
						class:in={mounted}
						style="--accent: {ASSESS_GREEN}; --delay: {i * 70}ms;"
						role="button"
						tabindex="0"
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-brand-navy/40 p-4">
		<div class="w-full max-w-2xl rounded-xl bg-white p-6 shadow-sm">
			...
			<input
				id="mel-plan-name"
				type="text"
				class="mb-4 w-full rounded border border-brand-navy/20 px-3 py-2 font-body outline-none focus:border-[#16a34a] focus:ring-2 focus:ring-[#16a34a]/20"
